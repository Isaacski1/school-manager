...
const skillOptions = ['Excellent', 'Very Good', 'Good', 'Fair', 'Poor'] as const;
...
const EditSkills = () => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        studentsList.forEach(s => {
          const found = existingSkills.find(sk => sk.studentId === s.id && sk.term === dynamicTerm);
          if (found) {
  ...
  const handleSkillChange = (studentId: string, field: keyof StudentSkills, value: string) => {
    setSkills(prev => ({
      ...prev,
      [studentId]: {
        ...prev[studentId],
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await db.addSystemNotification(
        `${user?.name} updated skills for ${className}.`,
        'assessment'
      );
              ...
              <thead className="bg-slate-100 text-slate-600 font-semibold">
                <tr>
                  ...
                  {skillFields.map(field => (
                    <th key={field.key} className="px-2 py-3 text-center">{field.label}</th>
                  ))}
                </tr>
              </thead>
                    ...
                    {skillFields.map(field => (
                      <td key={field.key} className="px-2 py-3">
                        <select
                          ...
                          onChange={(e) => handleSkillChange(student.id, field.key, e.target.value)}
