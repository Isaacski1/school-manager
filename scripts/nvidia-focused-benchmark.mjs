...
async function runTest(name, requestBody, stream = false) {
  ...
  try {
    ...
    if (result.reasoning) {
      console.log(`  Reasoning present: YES (${result.reasoning.length} chars)`);
      console.log(`  Reasoning preview: ${result.reasoning}...`);
    } else {
      console.log(`  Reasoning present: NO`);
    }
...
async function run() {
  console.log("NVIDIA Investigation - Focused Tests");
  console.log(`Model: ${CURRENT_MODEL}`);
  console.log(`Key: ${NVIDIA_API_KEY.substring(0, 10)}...`);
  
  const results = {};
  
  // Test 1: Current config with tools
  console.log("\n=== Test 1: Current config (with tools, required) ===");
  results.current = await runTest("Current config", {
    model: CURRENT_MODEL,
    messages: [
  ...
  // Test 2: Current config without tools
  console.log("\n=== Test 2: Current config (no tools) ===");
  results.noTools = await runTest("No tools", {
    model: CURRENT_MODEL,
    messages: [
      { role: "system", content: SYSTEM_PROMPT },
      { role: "user", content: "Hello" },
  ...
  console.log("\n=== Test 3: Minimal prompt ===");
  results.minimal = await runTest("Minimal", {
    model: CURRENT_MODEL,
    messages: [
  ...
  // Test 5: No reasoning
  console.log("\n=== Test 5: No reasoning ===");
  results.noReasoning = await runTest("No reasoning", {
    model: CURRENT_MODEL,
    messages: [
      { role: "system", content: SYSTEM_PROMPT },
      { role: "user", content: "Hello" },
