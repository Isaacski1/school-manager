...
const NVIDIAD_ENDPOINT = "https://integrate.api.nvidia.com/v1/chat/completions";
...
export class NvidiaProvider {
  ...
  async complete({ systemInstruction, messages, temperature = 0.2, maxOutputTokens = 900, timeoutMs = 15000, tools = null, toolChoice = null, telemetryContext = {} }) {
    ...
    const providerRequestId = `nvidia_${Date.now().toString(36)}_${crypto.randomBytes(4).toString("hex")}`;

    if (!this.apiKey) {
      const error = buildNormalizedError({
        category: "auth",
        message: "NVIDIA API key is not configured",
        code: "NVIDIA_AI_NOT_CONFIGURED",
        status: null,
    ...
    try {
      ...
      try {
        ...
        response = await fetch(NVIDIAD_ENDPOINT, {
          method: "POST",
          headers: {
        ...
        if (!response.ok) {
          ...
          const error = buildNormalizedError({
            ...
            code: response.status === 429 ? "NVIDIA_AI_QUOTA" : response.status === 401 || response.status === 403 ? "NVIDIA_AI_AUTH" : response.status === 404 ? "NVIDIA_AI_MODEL_UNAVAILABLE" : "NVIDIA_AI_REQUEST_FAILED",
      ...
      const normalizedError = buildNormalizedError({
        ...
        code: error?.code || `NVIDIA_AI_${category.toUpperCase()}`,
